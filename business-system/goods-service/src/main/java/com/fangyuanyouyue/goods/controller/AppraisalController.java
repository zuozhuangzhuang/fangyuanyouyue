...
public class AppraisalController extends BaseController{
    ...
    @ApiOperation(value = "申请鉴定", notes = "申请鉴定分为四种情况：1.卖家对自己商品进行鉴定，可显示到商品详情中 2.买家对别人的商品进行鉴定，只能自己看到 3.用户上传图片鉴定图片中的物品 4.官方认证店铺中的所有商品都是已鉴定",response = ResultUtil.class)
    @ApiImplicitParams({
            @ApiImplicitParam(name = "token", value = "用户token", required = true, dataType = "String", paramType = "query"),
            @ApiImplicitParam(name = "goodsIds", value = "商品ID数组", dataType = "int", paramType = "query"),
            @ApiImplicitParam(name = "title", value = "鉴定标题", required = true, dataType = "int", paramType = "query"),
            @ApiImplicitParam(name = "price", value = "鉴定赏金",  dataType = "int", paramType = "query"),
            @ApiImplicitParam(name = "description", value = "描述",  dataType = "int", paramType = "query"),
            @ApiImplicitParam(name = "imgUrl", value = "图片地址",  dataType = "String", paramType = "query"),

    })
    @PostMapping(value = "/addAppraisal")
    @ResponseBody
    public String addAppraisal(GoodsParam param) throws IOException {
        try {
            log.info("----》申请鉴定《----");
